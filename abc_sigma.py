...
if __name__ == '__main__':
    ...
    ax1.plot(sigma_array, a_array[:, 0], lw = 2, color = 'red')
    ax1.fill_between(sigma_array, a_array[:, 0] - a_array[:, 1], a_array[:, 0] + a_array[:, 1], color = 'red', alpha = 0.2)
    ...
    ax2.plot(sigma_array, b_array[:, 0], lw = 2, color = 'blue')
    ax2.fill_between(sigma_array, b_array[:, 0] - b_array[:, 1], b_array[:, 0] + b_array[:, 1], color = 'blue', alpha = 0.2)
    ax2.set_xlabel('sigma')
    ax2.set_ylabel('b value')
    ax2.grid()

    plt.legend(loc = 0)
    plt.savefig('a_b_sigma.png', bbox_inches = 'tight')
    plt.clf()
    plt.cla()
